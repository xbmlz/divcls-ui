...
const getTypeShortcut = (name: string) => {
  const base = `bg-${name} c-${name}-text b-1px b-solid b-transparent`
  ...
  const group = `bg:(fc:(border-rd-tr-0 border-rd-br-0) lc:(border-rd-tl-0 border-rd-bl-0) nfnl:(rd-0) nlc:(border-r-${name}-hover))`
  ...
  return `${base} ${hover} ${active} ${disabled} ${group} ${outline} ${link}`
}
...
export const buttonShortcuts: Exclude<Preset['shortcuts'], undefined | StaticShortcutMap> = [
  ...
  ['btn-group', 'inline-flex items-center'],
